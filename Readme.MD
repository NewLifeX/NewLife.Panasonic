...
松下PLC协议。基于 NewLife.IoT 标准框架，为 .NET 应用提供松下 PLC 设备的多种通信驱动（Modbus TCP / Modbus RTU / Mewtocol TCP）。**全部功能已完成并通过测试验证。**

## 亮点

- **三协议合一**：同一驱动入口自动选择 Modbus TCP / Modbus RTU，Mewtocol 独立驱动，覆盖松下 PLC 主流通信场景
- **连接可靠**：内建心跳检测和自动重连，适用于工业自动化场景
- **生态协同**：基于 NewLife.IoT 标准接口，与 IoTEdge、ZeroIoT 平台即插即用
- **MIT 许可**：最宽松开源许可，商业使用无限制
...
### Modbus RTU（串口连接，同一驱动自动识别）

```csharp
// 同一驱动，填写串口参数即可自动使用 RTU 协议
var driver = new PanasonicDriver();
var param = new PanasonicParameter
{
    PortName = "COM3",
...
// 打开通道（驱动自动判断：PortName 非空 → Modbus RTU）
var node = driver.Open(null, param);
```

### Mewtocol 原生协议（TCP 直连）

```csharp
// 创建 Mewtocol 驱动（适用于仅支持 Mewtocol 协议的松下 PLC）
...
| 驱动 | 协议 | 连接方式 | 注册名 | 
|------|------|----------|--------|
| `PanasonicDriver` | Modbus TCP / Modbus RTU | 以太网或串口（参数自动选择） | `PanasonicPLC` |
| `MewtocolDriver` | Mewtocol 原生 | 以太网（直连 PLC） | `PanasonicMewtocol` |

## 文档

| 文档 | 说明 |
|------|------|
| [功能说明](Doc/需求文档.md) | 愿景、核心功能与能力边界（v3.0） |
| [功能清单](Doc/功能清单.md) | 全部 6 个功能点已完成（v3.0） |
| [架构设计](Doc/架构设计.md) | 分层、组件、流程与设计决策（v3.0） |
| [竞品分析报告](Doc/竞品分析报告.md) | 竞品对比、功能矩阵、差距分析与建议（v3.0） |
